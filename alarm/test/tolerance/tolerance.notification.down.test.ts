...
//     alarmService = new AlarmServiceImpl(new MongoDBSecurityRuleRepository())
//   })
//
...
//
//     const actualAnomalies: number = (await alarmService.getExceedings()).length
//     await new Promise(
//       (resolve): NodeJS.Timeout =>
//         setTimeout(resolve, intervals.reduce((acc: number, curr: number) => acc + curr, 0) + 2000)
//     )
//
//     const newAnomalies: number = (await alarmService.getExceedings()).length
