...
function getMeasurements() {
  RequestHelper.get(`${logHost}/measurements?limit=50`)
    .then(async (res: any) => {
      measurements.value = []
...
function getOutliers() {
  RequestHelper.get(`${logHost}/anomalies/outliers?limit=50`)
    .then(async (res: any) => {
      outliers.value = []
      for (let i = res.data.length - 1; i >= 0; i--) {
...
function getIntrusions() {
  RequestHelper.get(`${logHost}/anomalies/intrusions?limit=50`)
    .then(async (res: any) => {
      intrusions.value = []
      for (let i = res.data.length - 1; i >= 0; i--) {
