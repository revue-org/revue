...
export class MongoDBLocationRepository implements LocationRepository {
    ...
    getLocationById(locationId: LocationId): Promise<Location> {
        return this._model.findOne({ locationId: locationId.value }).lean().then(location => LocationDBAdapter.asDomainEntity(location as LocationDBEntity));
    }

    getBuildings(): Promise<Location[]> {
        return this._model.find({ isRoom: false }).lean().then(locations => locations.map(location => LocationDBAdapter.asDomainEntity(location)));
    }

    getExernalBuildings(): Promise<Location[]> {
        return this._model.find({ isRoom: false, external: true }).lean().then(locations => locations.map(location => LocationDBAdapter.asDomainEntity(location)));
    }

    getBuildingRooms(buildingId: LocationId): Promise<Location[]> {
        return this._model.find({ buildingId: buildingId.value, isRoom: true }).lean().then(locations => locations.map(location => LocationDBAdapter.asDomainEntity(location)));
    }

    getRooms(): Promise<Location[]> {
        return this._model.find({ isRoom: true }).lean().then(locations => locations.map(location => LocationDBAdapter.asDomainEntity(location)));
    }

    getExternalRooms(): Promise<Location[]> {
        return this._model.find({ isRoom: true, external: true }).lean().then(locations => locations.map(location => LocationDBAdapter.asDomainEntity(location)));
    }
}